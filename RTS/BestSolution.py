...
from config.modelconfig import mnist,fashion,svhn,cifar_10,get_model_path
...
def dac_rank(x_select,nb_classes, dataname,select_pro, train_pro,x_train, y_train):
    ...
    suspicious_array, noise_array, correct_array = ob_sus_correct(sim_tolerate_num=5,
    ...
    for indexs in temp:
        # 获取的是索引
        if not indexs.any():continue
        scores = conquer(select_lable, select_pro, indexs, 100, nb_classes)
        # 获取索引+不确定度
        rank_list += scores
