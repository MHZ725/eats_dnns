...
from config.modelconfig import get_model_path,LeNet1,LeNet5,vgg16,resNet20
...
class CovInit(object):
    def __init__(self, X_train, Y_train, params: dict):
        if "model_path" in params.keys():
            ...
            model_path = get_model_path(params["data_name"], params["model_name"])
    ...
    def init_lsc_config(self, model_name, model):
        layer_conf = {LeNet5: ("dense", 8, 2000, 1e-5),  # 8
                      LeNet1: ("dense", 8, 2000, 1e-5),  # 8
                      resNet20: ('conv', 17, 100, 1e-4),  # 17
                      vgg16: ('conv', 7, 100, 1e-4),
                      }
...
class CovRank(object):
    ...
    def cal_nac_cov(self, only_ctm=False):
        input_layer, layers = self.get_layers()
        nac = metrics.nac(self.x_s, input_layer, layers, 0.75)
    ...
    def cal_tknc_cov(self, k=1):  # 1,2,3
        ...
        rank_lst2 = None
        return rate, tknc.t_collection, rank_lst, tknc.t_cam, rank_lst, None
