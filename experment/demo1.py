...
if __name__ == '__main__':
    ...
    for j in range(4):
        ...
        if ex_dataset == mnist:
            ...
            x_train=aug.x_train
            y_train=aug.y_train
        elif ex_dataset == cifar_10:
            aug=augCifar()
            data_num=40000
            x_train=aug.x_train
            y_train=aug.y_train
        elif ex_dataset == fashion:
            aug=augFashion()
            data_num=40000
            x_train=aug.x_train
            y_train=aug.y_train
        elif ex_dataset == svhn:
            aug=augSvhn()
            data_num=104128
            x_train=aug.x_train
            y_train=aug.y_train
        ...
        for select_rate in select_list:
            filename='result2\output_'+ex_dataset+'_'+ex_model+'_'+str(select_rate)+'.txt'
            print_color_file("{},{},{}".format(ex_dataset,ex_model,select_rate),"blue")
            for i in range(3):  #每次实验执行3次
                ...
                print_color_file("select rate:{},count:{}".format(select_rate,i),"yellow")
                ...
                cov_initer = get_cov_initer(x_train,y_train,ex_dataset,ex_model)
                cov_ranker = CovRank(cov_initer, model_path, x_aug, y_aug)
                _,_,_,_,rank,_ =cov_ranker.cal_tknc_cov(False)
