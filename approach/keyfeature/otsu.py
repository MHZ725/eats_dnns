...
def otsu_implementation(img_title="test.jpg", is_normalized=False, is_reduce_noise=False):
...
def call_otsu_threshold(img_title="test.jpg", is_reduce_noise=False):
    # Read the image in a greyscale mode
    image = cv2.imread(img_title, 0)
    ...
    plt.savefig("image_hist_test.png")
    plt.close()
    ...
    plt.savefig("image_hist_test_result.png")
    plt.close()
